	...
	class Wp_Usage_Feedback {
		...
		public function get_data() {
			...
			if( empty( $plugin ) ) {
				...
				$body['message'] .= esc_html( 'We can\'t detect any plugin information. This is most probably because you have not included the code in the plugin main file.', 'eddchatbot' );
		...
		public function form_default_text() {
			$form = array();
			$form['heading'] = esc_html( 'Sorry to see you go', 'eddchatbot' );
			$form['body'] = esc_html( '', 'eddchatbot' );
			$form['options'] = array(
				__( 'Found a Bug', 'eddchatbot' ),
				__( 'Need More Features', 'eddchatbot' ),
				__( 'Deactivating Temporarily', 'eddchatbot' ),
				__( 'Upgrading to Pro', 'eddchatbot' ),

			);
			$form['email'] = esc_html( 'Please provide email so we can contact with bug fixes', 'eddchatbot' );
			$form['details'] = esc_html( 'Please provide some details so we can improve the plugin', 'eddchatbot' );
			return $form;
		}
		...
		public function goodbye_ajax() {
			...
			$html .= '<p class="deactivating-spinner"><span class="spinner"></span> ' . esc_html( 'Submitting form', 'eddchatbot' ) . '</p>';
			?>
