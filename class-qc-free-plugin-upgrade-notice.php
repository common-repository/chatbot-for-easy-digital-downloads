	...
	class QcPluginUpgradeToProNotice {
		...
		function func_show_upgrade_link_with_action_links( $links )
		{
			$links = array_merge( $links, array(
				'<a title="'.$this->link_text.'" class="'.$this->link_class.'" style="font-weight: bold; color: '.$this->link_color.';" href="' . esc_url( $this->upgrade_link ) . '" target="'.$this->link_target.'">' . esc_html( $this->link_text, 'eddchatbot' ) . '</a>'
			) );
		...
		function func_show_upgrade_link_with_meta_links( $links, $file )
		{
			...
			if ( strpos( $file, "$this->plugin_main_file" ) !== false ) {
				...
				$new_links = array(
					'<a class="'.$this->link_class.'" style="font-weight: bold; color: '.$this->link_color.';" href="' . esc_url( $this->upgrade_link ) . '" title="'.$this->link_text.'" target="'.$this->link_target.'">' . esc_html( $this->link_text, 'eddchatbot' ) . '</a>'
				);
...
$instance_EDDBot = new QcPluginUpgradeToProNotice();
...
if( is_admin() )
{
    ...
    $instance_EDDBot->upgrade_link = "https://www.quantumcloud.com/products/woocommerce-chatbot-EDDBot/";
    ...
    $instance_EDDBot->plugin_slug = 'eddbot-chatbot'; //Plugin Slug. i.e. Folder Name
    $instance_EDDBot->plugin_main_file = 'qcld-EDDBot.php'; //Primary file of the pluign
    $instance_EDDBot->plugin_menu_slug = 'eddbot'; //Main Menu Slug

    $instance_EDDBot->show_with_action_links = true; //show in plugin.php page
    $instance_EDDBot->show_with_meta_links = true; //show in plugin.php page
    $instance_EDDBot->show_with_plugin_menu = true; //show inside parent menu of the plugin

    if( $instance_EDDBot->check_if_plugin_page() )
    {
        $instance_EDDBot->hook_with_plugin_action_links();
        $instance_EDDBot->hook_with_plugin_meta_links();
    }

    $instance_EDDBot->hook_with_plugin_submenu();
